...
def run_chat_mode():
    ...
    while True:
        ...
        elif "акумулятор" in phrase or "батаре" in phrase or "18650" in phrase:
        ...
        # 3. Режими роботи та характеристики (Що вміє робити)
            ...
            speak("Я маю три розумні режими! Перший — вільне керування через голос або веб-інтерфейс, де можна контролювати двигуни, "
                  ...
                  "Третій — автономне уникнення перешкод. Також я підтримумую трансляцію першої особи еф пі ві та колірний трекінг об'єктів.")
        ...
        # 4. Програмування та Команди
            ...
            speak("Увесь мій код та алгоритми написані на мові програмування Python 3.")
        ...
        # 5. Проблеми та шляхи вирішення (Охолодження, Wi-Fi, Датчики)
            ...
            speak("Мій процесор дуже потужний і при роботі з комп'ютерним зором нагрівається вище 80 градусів. Через брак місця під платою розширення, "
                  "стандартний кулер не влазить. Проблема вирішується тонкими мідними радіаторами, подовжувачами роз'єму GPIO "
        ...
        # --- СТАНДАРТНА ВІДПОВІДЬ, ЯКЩО ФРАЗИ НЕМАЄ В БАЗІ ДАНИХ ---
        else: 
            speak("Цікаве питання! Запитай мене краще про мої датчики, режими роботи, плату Raspberry Pi або про мою творчиню Надію.")
